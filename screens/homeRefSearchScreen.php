			...
			<div class="round5" style="height:5%;">
				...
				<form action="../DeleteValid.php" method="POST">
					...
					if (isset($_POST['refs'])) {
						...
						$enteredval = strtolower( trim($_POST['refs']) );

						while ($count < $total AND $row1 = mysqli_fetch_array($result1)) {



							if ((strtolower(trim($row1['NAME']))) == $enteredval ) {
								$CN++;
								...
								<?php 
							}elseif((strtolower(trim($row1['ADDRESS']))) == $enteredval ) {
								$CN++;
								?>
								<div class="d-flex flex-row" style="background-color: black;">
							...
							}elseif((strtolower(trim($row1['REF_ID']))) == $enteredval ) {
								$CN++;
								?>
								<div class="d-flex flex-row" style="background-color: black;">
