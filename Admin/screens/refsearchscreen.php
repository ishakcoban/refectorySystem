...
<body>
	...
	<div class="section">
		...
		<div class="round">
			...
			<form action="./AllSearchActionScreen.php" method="POST">
			...
			if (isset($_POST['refsrc'])) {

				$ent = strtolower(trim($_POST['refsrc'])) ;
				...
				while ($count < $total and $row1 = mysqli_fetch_array($result9)) {
					if ($ent == strtolower(trim($row1['REF_ID']) ) ) {
						$c1++;
					}elseif ($ent == strtolower(trim($row1['NAME']) )) {
						$c2++;
					}elseif ($ent == strtolower(trim($row1['ADDRESS']) )) {
						$c3++;
					}
...
include '../DBconnect.php';
				$result222 = mysqli_query($conn,"select count(1) FROM REFECTORY");
				$row1 = mysqli_fetch_array($result222);

				$total2 = $row1[0];

				$result333 = mysqli_query($conn,"select REF_ID ,NAME ,ADDRESS FROM REFECTORY");

				$cnm = 0;

				$count = 0;
				while ($count < $total2 and $rowmeal = mysqli_fetch_array($result333)){
					if ($ent == strtolower(trim($rowmeal['REF_ID']))) {
						$cnm++;
						...
						<?php
					...
					} elseif ($ent == strtolower(trim($rowmeal['NAME']))) {
							$cnm++;?>
							<div class="d-flex flex-row">
								<div style="margin-right:9%" class="infos"><p  class="text-center "><?php echo $rowmeal['REF_ID'];?></p></div>
								<div class="infos1"><p class="text-center "><?php echo $rowmeal['NAME'];?></p></div>
							...
							<?php
						}elseif ($ent == strtolower(trim($rowmeal['ADDRESS']))) {
							$cnm++;?>
							<div class="d-flex flex-row">
								<div style="margin-right:9%" class="infos"><p  class="text-center "><?php echo $rowmeal['REF_ID'];?></p></div>
								<div class="infos1"><p class="text-center "><?php echo $rowmeal['NAME'];?></p></div>
